...
const int positive_sample_number = 27876;	// only from training
const int negative_sample_number = 85252;	// only from training
...
{
	int i, index, is_pos = 0;
	if (strcmp(filename, "positive.txt") == 0)
		is_pos = 1;
	FILE* fp_in;
	FILE** fp_out = (FILE**)malloc(sizeof(FILE*)*((is_pos == 1) ? positive_set_number : negative_set_number));
	int left_total = (is_pos == 1) ? positive_sample_number : negative_sample_number;
	int* count = (int*)malloc(sizeof(int)*((is_pos == 1) ? positive_set_number : negative_set_number));
	int subset_number = (is_pos == 1) ? positive_set_number : negative_set_number;
	char subset_filename[10];
	char temp[100000];
	const int length_of_buffer = 100000;
	int order = 0; // current subset_file on the disk to write onto
	int size_of_subset;	// in sample numbers per subset
	if (is_pos == 1)
		size_of_subset = positive_sample_number / positive_set_number;
	else
		size_of_subset = negative_sample_number / negative_set_number;
	memset(temp, '\0', sizeof(temp));
	memset(subset_filename, '\0', sizeof(subset_filename));
	fp_in = fopen(filename, "r");

	for (i = 0; i<subset_number; ++i)
	{
		if (is_pos == 1)
			sprintf(subset_filename, "pos%d.txt", i);
		else
			sprintf(subset_filename, "neg%d.txt", i);
		fp_out[i] = fopen(subset_filename, "a");
	}
	while (left_total>0)
	{
		index = rand() % subset_number;
		while (count[index] == size_of_subset)
			index = (index + 1) % subset_number;
		fgets(temp, length_of_buffer, fp_in);
		fputs(temp, fp_out[index]);
		count[index]++;
		left_total--;
	}
	for (i = 0; i<subset_number; ++i)
