...
public:
	...
	static double dotMLP(const double *s, const feature_node *x, double et, double bia)
	{
		double ret = 0;
		while(x->index != -1)
		{
	...
	static void axpyMLP(const double a, const feature_node *x, double *y,const  double et,const double bia)
	{
		double tmp = (a-bia)/et;
		while(x->index != -1)
		{
			
			y[x->index-1] += tmp*x->value;
			x++;
		}
	}
...
void solve_l2r_lr_dual(const problem *prob, double *w, double eps, double Cp, double Cn)
{
	...
	while (iter < max_iter)
	{
		...
		for (s=0; s<l; s++)
		{
			...
			feature_node * const xi = prob->x[i];
			ywTx = yi*sparse_operator::dotMLP(w, xi,et,bia);
			...
			if(inner_iter > 0) // update w
			{
				alpha[ind1] = z;
				alpha[ind2] = C-z;
				sparse_operator::axpyMLP(sign*(z-alpha_old)*yi, xi, w, et, bia);
			}
		}
...
double predict_values(const struct model *model_, const struct feature_node *x, double *dec_values)
{
	...
	for(; (idx=lx->index)!=-1; lx++)
	{
		...
		if(idx<=n)
			for(i=0;i<nr_w;i++)
				dec_values[i] += (0.9*w[(idx-1)*nr_w+i]*lx->value+0.0001);
